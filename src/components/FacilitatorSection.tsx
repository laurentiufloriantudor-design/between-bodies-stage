...
const FacilitatorSection = () => {
  ...
  return (
    <section id="facilitator" ref={sectionRef} className="relative py-24 md:py-32 px-6 md:px-12 overflow-hidden">
      <AttentionElement seed={40} radius={600} maxShift={18} opacityFloor={0.3} className="absolute top-20 left-[6%] w-24 h-24 bg-teal rounded-full animate-drift opacity-60" />
      <AttentionElement seed={41} radius={600} maxShift={20} opacityFloor={0.3} className="absolute bottom-16 right-[12%] w-4 h-28 bg-coral animate-drift-slow" />
      ...
      <div className={`relative z-10 grid grid-cols-1 md:grid-cols-3 gap-12 transition-all duration-700 ${visible ? 'opacity-100' : 'opacity-0'}`}>
        <AttentionElement seed={42} radius={500} maxShift={14} opacityFloor={0.6} scaleRange={[0.98, 1.01]}>
        ...
        <div className={`md:col-span-2 ${visible ? 'animate-reveal-up animate-delay-2' : ''}`}>
          <AttentionElement seed={43} radius={400} maxShift={4} opacityFloor={0.7}>
            <span className="font-display text-sm tracking-[0.3em] text-coral block mb-6">Facilitator</span>
          </AttentionElement>

          <AttentionElement seed={44} radius={500} maxShift={8} opacityFloor={0.65} scaleRange={[0.992, 1.005]}>
          ...
          <AttentionElement seed={45} radius={450} maxShift={6} opacityFloor={0.6}>
          ...
          <AttentionElement seed={46} radius={400} maxShift={5} opacityFloor={0.55}>
